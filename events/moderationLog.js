  ...
  try {
    ...
    let modCaseNo = parseInt(guildSettings.modCaseNo), color,
      logData = [
        ...
        {
          name: 'Razão',
          value: reason || 'Nenhuma razão dada'
        },
        {
    ...
    switch (action.toLowerCase()) {
      ...
      */
      ...
      default:
        return guild.client.log.error(`O registro de moderação não está presente para ação ${action} .`);
    }
    ...
    modLogChannel.send({
      embed: {
        ...
        fields: logData,
        footer: {
          text: `Número do processo: ${modCaseNo}`
        },
        timestamp: new Date()
      }
    }).catch(e => {
