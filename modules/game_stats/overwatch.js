...
exports.exec = (ESXBot, message, args) => {
  ...
  if (!/^(us|eu|kr|cn)$/.test(args[0].toLowerCase())) {
     ...
     * @fires error
     */
    return ESXBot.emit('error', ESXBot.strings.error(message.guild.language, 'invalidInput'), ESXBot.strings.error(message.guild.language, 'invalidRegion', true, '`US`, `EU`, `KR` and `CN`'), message.channel);
  }
  if (!/^\w{3,12}(#|-)\d{4,6}$/.test(args[1])) {
     ...
     * @fires error
     */
    return ESXBot.emit('error', ESXBot.strings.error(message.guild.language, 'invalidInput'), ESXBot.strings.error(message.guild.language, 'invalidInput', true, 'BattleTag'), message.channel);
  }
  ...
  ow.getAll('pc', args[0], args[1].replace('#', '-')).then(data => {
    ...
    message.channel.send({
      embed: {
        ...
        image: {
          url: `https://resources-esx.github.io/images/overwatch/heros/${data.competitive.global.masteringHeroe || data.quickplay.global.masteringHeroe}.png`
        }
      }
    }).catch(e => {
      ESXBot.log.error(e);
    ...
    if (e.stack.includes('PROFILE_NOT_FOUND')) {
       ...
       * @fires error
       */
      return ESXBot.emit('error', ESXBot.strings.error(message.guild.language, 'notFound'), ESXBot.strings.error(message.guild.language, 'notFound', true, 'player'), message.channel);
    }
    ESXBot.log.error(e);
  });
};
