...
exports.exec = async (ESXBot, message, args) => {
  try {
    ...
    if (!seasonRegExp.test(args.season)) {
      return ESXBot.emit('error', ESXBot.strings.error(message.guild.language, 'invalidInput'), `You have entered an invalid season. Valid seasons are: ${validSeasons.join(', ').toUpperCase()}`, message.channel);
    }
    ...
    if (!regionRegExp.test(args.region)) {
      return ESXBot.emit('error', ESXBot.strings.error(message.guild.language, 'invalidInput'), ESXBot.strings.error(message.guild.language, 'invalidRegion', true, availableRegions.join(', ').toUpperCase()), message.channel);
    }
    ...
    if (!modeRegExp.test(args.mode)) {
      return ESXBot.emit('error', ESXBot.strings.error(message.guild.language, 'invalidInput'), `You have entered an invalid game mode. Valid game modes are: ${gameModes.join(', ').toUpperCase()}`, message.channel);
    }

    let options = {
      headers: {
        'Authorization': `Bearer ${ESXBot.credentials.PUBGAPIKey}`
      },
    ...
    if (response.data && response.data.length) {
      ...
      ESXBot.emit('error', 'PUBG API:', response.error, message.channel);
    }
    else {
      ESXBot.log.error(response);
    }
  }
  catch (e) {
    if (e.response) {
