...
exports.exec = (ESXBot, message, args) => {
  ...
  if (!/^(uplay|ps4|xone)$/.test(args[0] = args[0].toLowerCase())) {
     ...
     * @fires error
     */
    return ESXBot.emit('error', ESXBot.strings.error(message.guild.language, 'invalidInput'), ESXBot.strings.error(message.guild.language, 'invalidPlatform', true, '`Uplay`, `PS4` and `XOne`'), message.channel);
  }
  if (!/^[a-zA-Z][\w-. ]{2,14}$/.test(args[1] = args.slice(1).join(' '))) {
     ...
     * @fires error
     */
    return ESXBot.emit('error', ESXBot.strings.error(message.guild.language, 'invalidInput'), ESXBot.strings.error(message.guild.language, 'invalidInput', true, 'username'), message.channel);
  }
