...
exports.exec = async (ESXBot, message, args) => {
  try {
    ...
    let options = {
      headers: {
        'X-API-Key': `${ESXBot.credentials.bungieAPIKey}`
      },
    ...
    if (player.ErrorCode !== 1) {
      return ESXBot.emit('error', player.ErrorStatus, player.Message, message.channel);
    }
    if (!player.Response.length) {
      return ESXBot.emit('error', 'Not Found', 'No players were found for the given name.', message.channel);
    }
    ...
    if (stats.ErrorCode !== 1) {
      return ESXBot.emit('error', stats.ErrorStatus, stats.Message, message.channel);
    }
