...
exports.exec = async (ESXBot, message, args) => {
  try {
    ...
    if (!message.guild.warns) {
      ...
      * @fires error
      */
      return ESXBot.emit('error', ESXBot.strings.error(message.guild.language, 'notFound'), 'Everyone is decent here, no one has been warned.', message.channel);
    }
    if (!message.guild.warns.hasOwnProperty(user.id)) {
      ...
      * @fires error
      */
      return ESXBot.emit('error', ESXBot.strings.error(message.guild.language, 'notFound'), `Nah, ${user.tag} is good guy, he hasn't been warned.`, message.channel);
    }
    ...
    message.channel.send({
      embed: {
        ...
        description: ESXBot.strings.info(message.guild.language, 'clearWarn', message.author.tag, user.tag, args.reason)
      }
    }).catch(e => {
      ESXBot.log.error(e);
    });
