  ...
  try {
    ...
    if (message.guild.members.has(user.id)) {
      ...
      if (!member.bannable) {
        ...
        * @fires error
        */
        return ESXBot.emit('error', ESXBot.strings.error(message.guild.language, 'forbidden'), ESXBot.strings.error(message.guild.language, 'noPermission', true, 'ban', user), message.channel);
      }
    }
    ...
    message.channel.send({
      embed: {
        ...
        description: ESXBot.strings.info(message.guild.language, 'ban', message.author.tag, user.tag, args.reason),
        footer: {
          text: `ID ${user.id}`
        }
      }
    ...
    */
    ...
    DMChannel.send({
      embed: {
        color: ESXBot.colors.RED,
        description: ESXBot.strings.info(message.guild.language, 'banDM', message.author.tag, message.guild.name, args.reason)
      }
    }).catch(e => {
      ESXBot.log.error(e);
    });
