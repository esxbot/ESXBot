  ...
  try {
    ...
    if (msgs.size < 2 || msgs.length < 2) {
      ...
      if ((msgs.size === 1 || msgs.length === 1) && (user || args.includes('--bots'))) {
        error = ESXBot.strings.error(message.guild.language, 'singleMessage', true);
      }
      else {
        error = ESXBot.strings.error(message.guild.language, 'noDeletableMessage', true);
      }
      ...
      * @fires error
      */
      return ESXBot.emit('error', '', error, message.channel);
    }
    ...
    * @fires moderationLog
    */
    ESXBot.emit('moderationLog', message.guild, message.author, this.help.name, message.channel, reason, {
