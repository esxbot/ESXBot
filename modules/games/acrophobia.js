...
exports.exec = async (ESXBot, message) => {
  try {
    if (activeChannels.hasOwnProperty(message.channel.id))  {
      ...
      * @fires error
      */
      return ESXBot.emit('error', ESXBot.strings.error(message.guild.language, 'busy'), ESXBot.strings.error(message.guild.language, 'isGameInUse', true, 'acrophobia'), message.channel);
    }
    ...
    let acroLen = ESXBot.functions.getRandomInt(2, 5);
    let acronym = [];
