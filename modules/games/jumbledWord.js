...
exports.exec = async (ESXBot, message) => {
  try {
    if (activeChannels.includes(message.channel.id))  {
       ...
       * @fires error
       */
      return ESXBot.emit('error', ESXBot.strings.error(message.guild.language, 'busy'), ESXBot.strings.error(message.guild.language, 'isGameInUse', true, 'jumbled word'), message.channel);
    }
